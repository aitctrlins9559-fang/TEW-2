...
export const StockDetailModal: React.FC<StockDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[90] w-full h-[100dvh] bg-slate-100 flex flex-col text-slate-900 overflow-hidden overscroll-none animate-fadeIn select-none modal-backdrop">
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 max-w-2xl mx-auto w-full space-y-2.5 sm:space-y-3 overscroll-contain modal-content-scroll">
        {/* Price & Real-time Hero Banner */}
        <div className="bg-white border border-slate-200/90 p-4 rounded-2xl space-y-2 shadow-2xs text-slate-900">
          ...
          {(stock.dayHigh || stock.dayLow) && (
            <div className="pt-2 border-t border-slate-100 flex justify-between items-center text-xs font-mono text-slate-600">
        ...
        <div className="grid grid-cols-2 gap-2.5 text-xs">
          {/* Market Value */}
          <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 space-y-1 shadow-2xs">
            <span className="text-[11px] text-slate-500 font-sans block flex items-center gap-1.5 font-bold">
              <DollarSign className="w-3.5 h-3.5 text-indigo-600" /> 持有總市值 (NT$)
            </span>
            ...
            <div className="text-[11px] text-slate-500 font-mono border-t border-slate-100 pt-1 mt-1">
          ...
          <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 space-y-1 shadow-2xs">
            <span className="text-[11px] text-slate-500 font-sans block flex items-center gap-1.5 font-bold">
              <TrendingUp className="w-3.5 h-3.5 text-emerald-600" /> 未實現損益 (NT$)
            </span>
            <div className={`text-base sm:text-lg font-bold font-mono ${profitColorClass}`}>
              {profitTWD === null
                ? '--'
                : `${profitTWD >= 0 ? '+' : ''}${formatMoney(profitTWD, isPrivacy)}`}
            </div>
            <div className="text-[11px] text-slate-500 font-mono border-t border-slate-100 pt-1 mt-1 truncate">
              {costDetails.netProfitTWD !== undefined && (
                <span>淨利: <strong className={netProfitColorClass}>{costDetails.netProfitTWD >= 0 ? '+' : ''}{formatMoney(costDetails.netProfitTWD, isPrivacy)}</strong></span>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 space-y-1 shadow-2xs">
            <span className="text-[11px] text-slate-500 font-sans block flex items-center gap-1.5 font-bold">
              <Layers className="w-3.5 h-3.5 text-indigo-600" /> 持有股數與均價
            </span>
          ...
          {/* Dividends */}
          <div className="bg-emerald-50/80 p-3.5 rounded-xl border border-emerald-200/80 space-y-1 shadow-2xs">
            ...
            {divInfo.stockDps > 0 && (
              <div className="text-[10px] text-purple-900 font-mono font-bold pt-1 border-t border-emerald-200/60 flex items-center justify-between">
        ...
        <div className="bg-white p-3.5 sm:p-4 rounded-2xl space-y-2.5 shadow-2xs border border-slate-200/90">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <span className="text-xs font-black flex items-center gap-1.5 text-indigo-900">
              <Receipt className="w-4 h-4 text-indigo-600" />
              預估交易成本與淨損益精算
            </span>
            {!isUS && (
              <div className="inline-flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100/80 px-2.5 py-1 rounded-lg border border-slate-200 text-xs transition">
                <span className="text-slate-500 font-medium text-xs">券商折扣:</span>
                <select
                  ...
                  className="bg-transparent text-indigo-700 font-bold outline-none cursor-pointer text-xs font-sans py-0 px-0.5"
                >
            ...
            <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono">
                ...
                <div className="bg-amber-50 p-2 rounded-xl border border-amber-200/80">
                  ...
                  <span className="text-amber-900 font-bold text-xs">${costDetails.totalCostTWD.toLocaleString()} NT$</span>
                  <span className="text-[9px] text-amber-700/80 block font-sans">手續費+證交稅</span>
                </div>
              </div>

              {/* Net P&L comparison bar */}
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200 text-xs flex items-center justify-between flex-wrap gap-2">
                <div>
        ...
        {(divInfo.pendingStockShares > 0 || divInfo.annualDividendPerShare > 0) && (
          <div className="bg-white border border-emerald-200/80 p-3.5 rounded-2xl space-y-2.5 shadow-2xs">
            ...
            {divInfo.singleDividendPerShare > 0 && onDeductCashDividendCost && (
              <div className="bg-slate-50 p-2.5 rounded-xl border border-indigo-100 flex items-center justify-between gap-2">
      ...
      {/* Action Buttons Toolbar (Sticky Bottom) */}
      <div className="p-3 bg-white border-t border-slate-200/90 shrink-0 max-w-2xl mx-auto w-full space-y-2 shadow-2xs">
        <button
          onClick={() => {
            playClickSound();
            onClose();
          ...
          className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-4 rounded-xl text-xs sm:text-sm transition flex items-center justify-center gap-2 btn-interact shadow-xs"
        >
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            className="bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 py-2 px-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1 btn-interact"
          >
          ...
          <button
            ...
            className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 py-2 px-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1 btn-interact"
          >
