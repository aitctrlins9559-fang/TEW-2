...
export const AICopilotModal: React.FC<AICopilotModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[90] w-full h-[100dvh] bg-slate-100 flex flex-col text-slate-900 overflow-hidden overscroll-none animate-fadeIn select-none modal-backdrop">
      ...
      {/* Navigation Tabs */}
      <div className="bg-slate-50 border-b border-slate-200/90 px-3 py-2 shrink-0">
        <div className="max-w-3xl mx-auto flex bg-white p-1 rounded-xl border border-slate-200/90 shadow-2xs">
          <button
            onClick={() => {
              playClickSound();
              setActiveTab('report');
          ...
          >
            <FileText className="w-3.5 h-3.5" /> 📊 全盤戰情報告
          </button>
          ...
          >
            <MessageSquare className="w-3.5 h-3.5" /> 💬 即時問答諮詢
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-5 max-w-3xl mx-auto w-full space-y-3 overscroll-contain modal-content-scroll">
          ...
          {!isLoading && !error && analysis && (
            <div className="space-y-3 text-xs sm:text-sm">
              {/* Header Rating */}
              <div className="bg-white p-3.5 rounded-2xl border border-slate-200/90 shadow-2xs flex flex-wrap justify-between items-center gap-2">
                <div>
              ...
              {/* Overall Summary */}
              <div className="bg-white border border-purple-200 p-4 rounded-2xl space-y-1.5 shadow-2xs">
              ...
              <div className="bg-white border border-slate-200/90 p-4 rounded-2xl space-y-1.5 shadow-2xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white border border-emerald-200/90 p-3.5 rounded-2xl space-y-1.5 shadow-2xs">
                ...
                <div className="bg-white border border-rose-200/90 p-3.5 rounded-2xl space-y-1.5 shadow-2xs">
              ...
              {/* Action Advice */}
              <div className="bg-white border border-amber-200 p-4 rounded-2xl space-y-1.5 shadow-2xs">
      ...
      {activeTab === 'chat' && (
        <div className="flex-1 flex flex-col p-3 sm:p-4 max-w-3xl mx-auto w-full overflow-hidden">
          {/* API Key Mode Header Bar */}
          <div className="bg-white p-2.5 rounded-2xl border border-slate-200/90 shadow-2xs text-xs shrink-0 space-y-2 mb-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-slate-700">
                ...
                <span className={`font-mono text-[11px] px-2 py-0.5 rounded-full border ${
                  ...
                  {customApiKey ? '自備 Google API Key' : '預設智庫代理模式'}
                </span>
              </div>
              ...
              >
                ⚙️ API 設定 {showApiKeyConfig ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
              </button>
            </div>

            {/* Expandable API Key Drawer */}
            {showApiKeyConfig && (
              <div className="p-3 bg-slate-50 rounded-xl border border-purple-200 space-y-2 animate-in fade-in duration-200">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white border border-slate-300 rounded-xl px-3 py-1.5 text-xs text-slate-900 font-mono focus:outline-none focus:border-purple-600"
                  />
                  <button
          ...
          <div className="flex flex-wrap gap-1.5 pb-2 shrink-0">
            {quickPrompts.map((qp, idx) => (
              <button
                ...
                disabled={isSending}
                className="text-[11px] bg-white hover:bg-purple-50 text-purple-800 border border-purple-200 px-2.5 py-1 rounded-xl transition hover:border-purple-300 text-left disabled:opacity-50 font-medium shadow-2xs"
              >
                {qp}
              </button>
            ))}
          ...
          <div className="flex-1 overflow-y-auto space-y-3 p-3 bg-white rounded-2xl border border-slate-200/90 shadow-2xs overscroll-contain modal-content-scroll">
          ...
          <form
            ...
            className="flex gap-2 pt-2.5 shrink-0"
          >
            <input
              type="text"
              value={inputMsg}
              ...
              disabled={isSending}
              className="flex-1 bg-white border border-slate-300 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm text-slate-900 focus:outline-none focus:border-purple-600 transition shadow-2xs"
            />
            <button
              type="submit"
              disabled={!inputMsg.trim() || isSending}
              className="bg-purple-600 hover:bg-purple-700 disabled:bg-slate-300 text-white font-bold px-4 py-2.5 rounded-xl text-xs transition flex items-center gap-1.5 btn-interact shrink-0 shadow-xs"
            >
