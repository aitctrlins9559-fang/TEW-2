...
export const SingleStockChart: React.FC<SingleStockChartProps> = ({
    ...
    return {
      ...
      scales: {
        x: {
          ...
          ticks: { color: '#64748b', maxTicksLimit: 7, font: { family: 'sans-serif', size: 10, weight: 'bold' as const } },
        },
        y: {
  ...
  return (
    <div
      ...
      className="glass-card p-4 sm:p-6 rounded-2xl sm:rounded-3xl space-y-4 border border-slate-200/90 shadow-sm bg-white"
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 pb-3.5">
        <div className="flex items-center justify-between w-full sm:w-auto gap-2">
          <h2 className="text-base font-bold text-slate-900 flex items-center gap-2 tracking-tight">
            ...
            個股即時分時走勢圖
            <span className="text-[10px] font-mono bg-emerald-50 text-emerald-700 border border-emerald-100 px-2 py-0.5 rounded-full flex items-center gap-1 font-bold">
              <Activity className="w-3 h-3 text-emerald-600 animate-pulse" /> 盤中現價同步
            </span>
          </h2>
          {onOpenFullModal && (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-indigo-50 border border-indigo-100 text-indigo-700 hover:bg-indigo-100 text-xs font-bold transition shrink-0 btn-interact"
              title="開啟滿版看盤"
            >
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 w-full sm:w-auto">
          ...
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-xs text-slate-800 font-bold outline-none cursor-pointer hover:border-indigo-300 transition w-full sm:w-auto"
          >
          ...
          <div className="relative w-full sm:w-auto">
            <form onSubmit={handleSearchSubmit} className="relative flex items-center w-full">
              <input
                ...
                className="glass-input rounded-xl px-3 py-1.5 text-xs text-slate-900 outline-none w-full sm:w-56 font-medium pr-7 focus:border-indigo-500"
              />
            ...
            {showResults && (
              <div className="absolute left-0 sm:left-auto sm:right-0 top-full mt-2 w-full sm:w-72 bg-white border border-slate-200 rounded-2xl shadow-xl max-h-60 overflow-y-auto z-50 divide-y divide-slate-100">
      ...
      {intradayData && (
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-slate-200/80 text-slate-700 uppercase">
          ...
          <div className="flex items-center gap-3 self-end sm:self-auto">
            <div className="text-right">
              <div className="text-[10px] text-slate-500 font-mono font-bold uppercase mb-0.5">
            ...
            <div
              className={`px-3 py-1.5 rounded-xl border font-mono font-bold flex flex-col items-center justify-center shrink-0 ${
                diff > 0
                  ? isRedUp
      ...
      {intradayData && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200">
            <div className="text-slate-500 font-medium text-[11px] mb-0.5">昨日收盤</div>
            <div className="text-sm font-bold text-slate-900 font-mono tabular-nums">
              ${intradayData.prevClose.toFixed(2)}
            </div>
          </div>
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200">
            <div className="text-slate-500 font-medium text-[11px] mb-0.5">當日最高</div>
            <div className="text-sm font-bold text-rose-600 font-mono tabular-nums">
              ${intradayData.highPrice.toFixed(2)}
            </div>
          </div>
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200">
            <div className="text-slate-500 font-medium text-[11px] mb-0.5">當日最低</div>
            <div className="text-sm font-bold text-emerald-600 font-mono tabular-nums">
              ${intradayData.lowPrice.toFixed(2)}
            </div>
          </div>
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200">
            <div className="text-slate-500 font-medium text-[11px] mb-0.5">當日振幅</div>
            <div className="text-sm font-bold text-amber-600 font-mono tabular-nums">
      ...
      {/* Line Chart Canvas */}
      <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200 h-64 sm:h-72 relative w-full pt-1">
        {loading ? (
      ...
      <div className="flex justify-between text-[10px] text-slate-500 font-mono tracking-wider uppercase tabular-nums border-t border-slate-100 pt-2">
