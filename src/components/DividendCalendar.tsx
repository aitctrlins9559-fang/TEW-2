    ...
    <div className="glass-card p-2 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-200/90 shadow-sm space-y-2.5 sm:space-y-4 bg-white relative w-full">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2.5 sm:gap-3 border-b border-slate-100 pb-2.5 sm:pb-3.5">
        ...
        <div className="grid grid-cols-4 lg:flex w-full lg:w-auto p-0.5 sm:p-1 bg-slate-100 rounded-lg sm:rounded-xl border border-slate-200/80 text-xs font-bold shrink-0 gap-0.5 sm:gap-1">
          ...
          >
            ...
            <span><span className="sm:hidden">總覽</span><span className="hidden sm:inline">總覽 & 月月配</span></span>
          </button>
          <button
            onClick={() => {
              playClickSound();
          ...
          >
            <BellRing className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
            <span><span className="sm:hidden">提醒({summary.upcomingReminders.length})</span><span className="hidden sm:inline">除息提醒 ({summary.upcomingReminders.length})</span></span>
          </button>
          <button
            onClick={() => {
              playClickSound();
          ...
          >
            ...
            <span><span className="sm:hidden">目標</span><span className="hidden sm:inline">自由目標與本金缺口</span></span>
          </button>
          <button
            onClick={() => {
              playClickSound();
          ...
          >
            <Repeat className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
            <span><span className="sm:hidden">複利</span><span className="hidden sm:inline">🚀 DRIP 複利模擬</span></span>
          </button>
        </div>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 p-2.5 sm:p-5 rounded-2xl border border-slate-200/80 space-y-3">
            <div className="grid grid-cols-12 gap-1 sm:gap-2 h-44 sm:h-48 items-end pt-6 sm:pt-8 pb-1 w-full">
              {summary.monthlyBreakdown.map((val, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center h-full justify-end group min-w-0">
                    <div className="text-[8px] sm:text-[10px] font-mono text-slate-600 font-bold opacity-80 sm:opacity-0 group-hover:opacity-100 transition mb-1 text-center truncate w-full">
                    ...
                    />
                    <div className="text-[9px] sm:text-[11px] font-mono font-bold text-slate-700 mt-1 sm:mt-2 truncate">
                      {monthNames[idx]}
                    </div>
                  </div>
                );
